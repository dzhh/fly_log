...
public class LogsDaoImpl implements LogsDao {
	...
	@Override
	public List<LoginLogModel> selectLoginLogList(Page page, String systemId, String username, DateInterval dateInterval) {
		...
		List<LoginLogModel> list = jdbcTemplate.query(sql, selectParams, new BeanPropertyRowMapper(LoginLogModel.class));
	...
	public List<OperationLogModel> selectOperationList(Page page, String systemId,DateInterval dateInterval) {
		...
		@SuppressWarnings({ "unchecked", "rawtypes" })
		List<OperationLogModel> list = jdbcTemplate.query(sql, selectParams, new BeanPropertyRowMapper(OperationLogModel.class));
		for(int i=0;i<list.size();i++){
			list.get(i).setOrderNumber(i+1);
		}
